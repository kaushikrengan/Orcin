...
import { Upload, CheckCircle2, ChevronRight, FileText, Database, Play, AlertTriangle, ShieldCheck, RefreshCw, Layers, Sliders, PlayCircle, Sparkles, ArrowUpRight, ChevronLeft, BookOpen, Award, Check } from "lucide-react";
...
// ==========================================
// 🏢 DEMO 4: APOLLO DYNAMIC COMPETENCY WORKSPACE
// ==========================================
export function ApolloDemo() {
  const [page, setPage] = useState<"hub" | "detail" | "chat">("hub");
  // Total progress starting value (e.g. 59% from the screenshot)
  const [totalProgress, setTotalProgress] = useState(59);

  const initialCompetencies = [
    {
      id: "rag",
      title: "RAG Architecture & Indexing",
      desc: "Vector databases, ingestion chunk sizes, query routing, and embeddings calculation.",
      score: 54,
      target: 85,
      completed: false,
    },
    {
      ...
      desc: "Empirical response scoring, synthetic dataset logging, and safety safeguards.",
      score: 68,
      target: 80,
      completed: false,
    },
    {
      ...
      desc: "STRIDE risk methodology, architectural logs auditing, and vulnerability patches.",
      score: 72,
      target: 80,
      completed: false,
    },
    {
      id: "tune",
      title: "Retrieval Fine-Tuning & Weights",
      desc: "Weight adjustment, context distilling, alignment reinforcement, and LLM bias control.",
      score: 40,
      target: 75,
      completed: false,
    }
  ...
  const [selectedCompId, setSelectedCompId] = useState<string>("rag");
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ sender: "user" | "apollo"; text: string }>>([
    { sender: "apollo", text: "Hello Adore, I am your always-on Competency Copilot. How can I help you accelerate your readiness today?" }
  ]);

  const selectedComp = competencies.find(c => c.id === selectedCompId) || competencies[0];
  ...
  const handleQuizSubmit = (opt: string) => {
    ...
    if (opt === "B") {
      setIsCorrect(true);
      // Increment the score for this competency to match/exceed target
      setCompetencies(prev => prev.map(c => {
        if (c.id === selectedCompId) {
          return { ...c, score: c.target, completed: true };
        }
        return c;
      }));
      // Boost total progress organically
      setTotalProgress(prev => Math.min(100, prev + 8));
    } else {
      setIsCorrect(false);
    }
  };
  ...
  const sendChatMessage = () => {
    ...
    setTimeout(() => {
      let reply = "I can guide you through that topic. Try launching the assessment to officially verify and update your score!";
      if (userMsg.toLowerCase().includes("rag")) {
        reply = "Vector databases segment documents into chunks. For RAG, choosing the ideal chunk size (e.g., 256-512 tokens) with adequate overlap minimizes context loss.";
      } else if (userMsg.toLowerCase().includes("safety") || userMsg.toLowerCase().includes("eval")) {
        reply = "Evaluating AI responsiveness requires robust empirical pipelines. We construct synthetic test beds and automate criteria validation to verify factual guardrails.";
      } else if (userMsg.toLowerCase().includes("security") || userMsg.toLowerCase().includes("threat")) {
        reply = "Security for AI systems demands a comprehensive Threat Model. Auditing architectural logs and shielding vector databases ensures against injection attacks.";
      }
      setChatMessages(prev => [...prev, { sender: "apollo", text: reply }]);
    }, 600);
  };

  return (
    <div className="w-full h-full bg-[#fcfcfb] text-graphite rounded-b-2xl overflow-hidden flex flex-col font-sans select-none text-xs">
      
      {/* Simulation Header */}
      <div className="shrink-0 bg-white border-b border-[#ecebe6] px-4 md:px-6 py-2.5 flex justify-between items-center z-10">
        {/* Associate User badge */}
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-teal/10 overflow-hidden flex-shrink-0 flex items-center justify-center border border-teal/20">
            <svg viewBox="0 0 24 24" className="w-4 h-4 text-teal fill-current">
              <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
            </svg>
          </div>
          <div className="flex flex-col leading-tight hidden sm:flex">
            <span className="font-semibold text-[10px] text-graphite font-sans">Associate Space</span>
            <span className="text-[9px] text-[#86837c] font-normal">Adore Patel / Hub</span>
          </div>
        </div>

        {/* Display Title */}
        <div className="font-mono text-xs sm:text-sm font-semibold tracking-[0.25em] text-graphite select-none uppercase">
          APOLLO
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <button 
            onClick={() => setPage(page === "chat" ? "hub" : "chat")}
            className="bg-[#0e1317] hover:bg-black font-sans font-medium text-[9px] sm:text-[10px] tracking-wide text-white px-2.5 sm:px-3 py-1.5 rounded-full flex items-center gap-1 sm:gap-1.5 shadow-sm transition-all cursor-pointer border-none"
          >
          ...
          <button 
            onClick={resetAll}
            className="border-none bg-transparent hover:bg-graphite/5 font-mono text-[8px] sm:text-[9px] font-semibold text-graphite/40 tracking-wider uppercase px-2 py-1.5 rounded-md transition-all cursor-pointer"
          >
            RESET
          </button>
        </div>
      ...
      <div className="flex-grow overflow-hidden flex flex-col relative text-[11px]">
        <AnimatePresence mode="wait">
          ...
          {page === "hub" && (
            <motion.div
              ...
              className="flex-grow overflow-y-auto px-4 py-5 sm:p-6 flex flex-col gap-5 bg-[#F6F5F2]"
            >
              <div className="flex flex-col">
                <h4 className="text-lg sm:text-2xl font-bold tracking-tight text-graphite font-heading">
                  Welcome back, Adore.
                </h4>
                <p className="text-[10px] sm:text-[11.5px] text-[#6c6962] font-sans font-light mt-1 max-w-2xl leading-relaxed">
                  Your current learning progress holds at <span className="font-semibold text-teal">{totalProgress}%</span>. Complete the assigned modules below to update your skill targets.
                </p>
              </div>

              {/* Subheader category indicator */}
              <div className="flex justify-between items-center border-b border-[#ecebe6] pb-2">
                <span className="font-sans text-[8.5px] sm:text-[10px] tracking-wider text-graphite/40 uppercase font-bold">
                  ASSIGNED SKILLS & MODULES
                </span>
                <span className="font-mono text-[9px] text-[#6c6962]/50 font-normal">
                  Count: {competencies.length} assignments
                </span>
              </div>

              {/* Grid of cards */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 flex-grow mb-4">
                {competencies.slice(0, 3).map((comp) => {
                  const targetMet = comp.score >= comp.target;
                  return (
                    <div
                      key={comp.id}
                      onClick={() => selectCompetency(comp.id)}
                      className="bg-white border border-[#ecebe6] hover:border-teal/40 hover:bg-[#FDFCFB]/80 hover:shadow-md rounded-xl p-4 flex flex-col justify-between cursor-pointer transition-all gap-4 group"
                    >
                      <div className="flex flex-col gap-1.5">
                        <div className="flex justify-between items-start gap-2">
                          <span className="font-semibold text-xs text-graphite leading-snug font-sans group-hover:text-teal transition-colors">
                            {comp.title}
                          </span>
                          <ArrowUpRight size={14} className="text-graphite/30 group-hover:text-teal group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all flex-shrink-0" />
                        </div>
                        <p className="text-[10px] text-[#7c7972] leading-relaxed font-sans font-light">
                          {comp.desc}
                        </p>
                      </div>
                      ...
                      <div className="flex flex-col gap-1.5 pt-2">
                        <div className="flex justify-between text-[9px] font-mono">
                          <span className={`${targetMet ? "text-teal font-semibold" : "text-graphite/50"}`}>
                            Completion score: <b className="font-sans text-xs">{comp.score}%</b>
                          </span>
                          <span className="text-graphite/40">
                            is {comp.target}% min
                          </span>
                        </div>
                        <div className="w-full h-1 bg-graphite/10 rounded-full overflow-hidden">
                          <motion.div 
                            className={`h-full rounded-full ${targetMet ? "bg-teal" : "bg-[#0e1317]"}`}
                            initial={{ width: 0 }}
          ...
          {page === "detail" && (
            <motion.div
              ...
              exit={{ opacity: 0, x: -10 }}
              transition={{ duration: 0.2 }}
              className="flex-grow overflow-y-auto p-4 sm:p-6 flex flex-col gap-5 bg-white"
            >
              {/* Back button breadcrumb */}
              <div className="flex justify-between items-center shrink-0">
                <button
                  onClick={() => setPage("hub")}
                  className="border-none bg-transparent hover:bg-graphite/5 text-[#5c5952] font-sans font-medium text-[10px] sm:text-xs py-1.5 px-3 rounded-lg flex items-center gap-1 transition-all cursor-pointer"
                >
                  <ChevronLeft size={14} /> Back to Hub
                </button>
                <div className="flex gap-2 items-center font-normal">
                  <span className="font-sans text-[9px] text-[#7c7972] uppercase tracking-widest">
                    MODULE DETAILS
                  </span>
                  <span className={`w-2 h-2 rounded-full ${selectedComp.score >= selectedComp.target ? "bg-teal" : "bg-yellow-400"}`} />
                </div>
              </div>

              {/* Module heading */}
              <div className="flex flex-col border-b border-graphite/5 pb-4">
                <h4 className="text-base sm:text-lg font-bold font-sans text-graphite">
                  {selectedComp.title}
                </h4>
                <p className="text-[10px] sm:text-xs text-[#6c6962] font-sans font-light mt-1 max-w-xl">
                  {selectedComp.desc}
                </p>
                <div className="flex gap-4 items-center mt-3 text-[10px] font-mono tracking-wide text-graphite/50">
                  <span>Current: <b className="text-graphite font-sans text-xs">{selectedComp.score}%</b></span>
                  <span>Target Competency: <b className="text-teal font-sans text-xs">{selectedComp.target}%</b></span>
                </div>
              </div>

              {/* Assessment Section */}
              <div className="flex-grow flex flex-col gap-4">
                <div className="bg-[#FAF9F6] border border-graphite/5 rounded-xl p-4 sm:p-5">
                  <div className="flex items-center gap-1.5 text-[#5c5952] font-semibold mb-3">
                    <Sparkles size={13} className="text-teal" />
                    <span className="font-mono text-[9px] tracking-wider uppercase">Apollo Interactive Assessment</span>
                  </div>

                  <p className="text-[11px] sm:text-xs font-semibold text-graphite/80 leading-relaxed max-w-2xl">
                    {selectedComp.id === "rag" ? (
                      "When orchestrating a high-precision RAG pipeline against unstructured legacy tables, which strategy best preserves context-retrieval fidelity?"
                    ) : selectedComp.id === "eval" ? (
                      "For automated safety alignment checkpoints, what criterion should take absolute priority during LLM contextual verification?"
                    ) : selectedComp.id === "threat" ? (
                      "How matches STRIDE threat alignment when securing vector endpoints against persistent prompt injections?"
                    ) : (
                      "To minimize parameter drift while fine-tuning downstream lightweight embed models, which approach yields optimal weights alignment?"
                    )}
                  </p>
                  ...
                  <div className="flex flex-col gap-2 mt-4">
                    {[
                      { key: "A", text: selectedComp.id === "rag" ? "Apply a uniform 500-token chunk scale across all tabular layouts blindly." : "Deploy speculative parallel tokens testing without continuous ground truth validation." },
                      { key: "B", text: selectedComp.id === "rag" ? "Execute intelligent semantic-overlay chunk splits with recursive parent context retrieval." : "Structure strict semantic evaluation guardrails built around ground-truth adversarial validation matrices." },
                      { key: "C", text: selectedComp.id === "rag" ? "Force fallback queries exclusively to standard, full-document fuzzy text scans." : "Authorize direct vector database overrides with no persistent auditing trail." }
                    ].map((opt) => {
                      const isOptionSelected = quizAnswer === opt.key;
                      let optionBg = "bg-white border-graphite/10 hover:border-teal/40 hover:bg-[#FDFCFB]/80";
                      
                      if (quizSubmitted) {
                        if (opt.key === "B") {
                          optionBg = "bg-teal/10 border-teal text-teal font-medium";
                        } else if (isOptionSelected) {
                          optionBg = "bg-red-50 border-red-200 text-red-500 font-medium";
                        } else {
                          optionBg = "bg-white border-graphite/5 opacity-60";
                        }
                      }
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left font-sans text-[10px] sm:text-xs p-3 rounded-lg border flex gap-3 transition-all cursor-pointer ${optionBg}`}
                        >
                          <span className={`w-5 h-5 rounded-md flex items-center justify-center font-mono text-[10px] font-bold shrink-0 ${
                            quizSubmitted && opt.key === "B" 
                              ? "bg-teal text-white" 
                              : isOptionSelected 
                              ? "bg-red-500 text-white" 
                              : "bg-[#ecebe6] text-graphite/60"
                          }`}>
                            {opt.key}
                          </span>
                  ...
                  <AnimatePresence>
                    {quizSubmitted && (
                      <motion.div
                        initial={{ opacity: 0, y: 5 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="mt-4 pt-4 border-t border-graphite/5 flex flex-col gap-1 leading-relaxed text-[10px] sm:text-[11px]"
                      >
                        {isCorrect ? (
                          <div className="flex flex-col gap-1.5">
                            <span className="font-semibold text-teal flex items-center gap-1">
                              <CheckCircle2 size={13} /> Correct Strategy Identified!
                            </span>
                            <p className="text-[#6c6962] font-sans font-light">
                              Your answer effectively retains structural tabular context. Competency has been automatically synchronized, updating your performance score to <b className="text-teal">{selectedComp.target}%</b>!
                            </p>
                            <button
                              onClick={() => setPage("hub")}
                              className="self-start mt-1 border-none bg-teal hover:bg-teal-dark text-white font-semibold px-4 py-1.5 rounded-md transition-all cursor-pointer shadow-sm text-[10px]"
                            >
                              Sync with Training Ledger
                            </button>
                          </div>
                        ) : (
                          <div className="flex flex-col gap-1">
                            <span className="font-semibold text-red-500">Incomplete Solution</span>
                            <p className="text-[#6c6962] font-sans font-light">
                              That strategy lacks the required parent-retrieval loop context or robust alignment filters. Reset or select another option to complete competency checkout.
                            </p>
                            <button
                              onClick={() => { setQuizAnswer(null); setQuizSubmitted(false); }}
                              className="self-start mt-1 border border-graphite/20 bg-transparent hover:bg-graphite/5 text-graphite font-semibold px-3 py-1.5 rounded-md transition-all cursor-pointer text-[10px]"
                            >
                              Retry Assessment
                            </button>
                          </div>
          ...
          {page === "chat" && (
            <motion.div
              ...
              className="flex-grow flex flex-col bg-[#F3F2EE]"
            >
              {/* Chat Sub-header */}
              <div className="shrink-0 bg-[#e7e5e0] border-b border-graphite/5 px-4 py-2.5 flex justify-between items-center">
                <div className="flex items-center gap-1.5 font-semibold text-graphite">
                  <Sparkles size={12} className="text-teal" />
                  <span className="font-mono text-[9px] tracking-wider uppercase bg-transparent">Apollo Learning Assistant</span>
                </div>
                <button
                  onClick={() => setPage("hub")}
                  className="border-none bg-transparent hover:bg-graphite/10 text-[#5c5952] font-mono text-[9px] px-2 py-1 rounded transition-all cursor-pointer"
                >
                  CLOSE
                </button>
              </div>
              ...
              {/* Messages Body */}
              <div className="flex-grow overflow-y-auto px-4 py-5 flex flex-col gap-3 font-sans">
                {chatMessages.map((msg, idx) => {
                  const isApollo = msg.sender === "apollo";
                  return (
                    <div
                      key={idx}
                      className={`flex flex-col gap-1 max-w-[85%] ${
                        isApollo ? "self-start" : "self-end items-end"
                      }`}
                    >
                      <span className="text-[8px] font-mono text-[#7c7972] uppercase">
                        {isApollo ? "Apollo Copilot" : "Adore"}
                      </span>
                      <div className={`p-3 rounded-xl leading-normal text-[10.5px] font-sans ${
                        isApollo 
                          ? "bg-white border border-[#ecebe6] text-graphite" 
              ...
              {/* Chat Input Bar */}
              <div className="shrink-0 bg-white border-t border-[#ecebe6] p-3 flex gap-2 items-center">
                <input
                  type="text"
                  placeholder="Ask a technical capability, e.g. 'How to build RAG?'"
                  value={chatInput}
                  onChange={(e) => setChatInput(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter") sendChatMessage(); }}
                  className="flex-grow border border-graphite/10 text-graphite rounded-lg px-3 py-1.5 text-[10.5px] outline-none font-sans focus:border-teal/50"
                />
                <button
                  onClick={sendChatMessage}
                  className="bg-teal hover:bg-teal-dark hover:shadow text-white font-semibold font-sans px-3.5 py-1.5 rounded-lg border-none cursor-pointer transition-all shrink-0 text-[10.5px]"
                >
                  Ask
                </button>
              </div>
      ...
      {/* Info Status strip at bottom */}
      <div className="shrink-0 bg-white border-t border-[#ecebe6] py-1 px-4 text-[8px] sm:text-[9px] font-mono text-graphite/40 select-none flex justify-between items-center leading-none">
        <span className="flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-teal" />
          LEDGER: SYNCED & VALIDATED
        </span>
        <span>AGENT_UID: APOLLO_V4.9.1</span>
      </div>
